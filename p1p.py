...
number_to_wav = {
    0: "0.wav",  # Added missing mapping for 0
    1: "1.wav",
    2: "2.wav",
    3: "3.wav",
    4: "4.wav",
    ...
    for file in files:
        ...
        try:
            if os.path.exists(filepath):
                ...
                logging.warning(f"Audio file not found: {file}")
...
# Function to construct the pronunciation of a number
def pronounce_number(number):
    if number in number_pronunciations:
        return number_pronunciations[number]
    
    if number < 20:
        return f"10.wav+KƐ.wav+{number % 10}.wav"
    
    elif number < 100:
        tens = (number // 10) * 10
        units = number % 10
        if units == 0:
            return f"20-90.wav+{tens // 10}.wav"
        else:
            return f"20-90.wav+{tens // 10}.wav+KƐ.wav+{units}.wav"
    
    elif number < 1000:
        hundreds = number // 100
        remainder = number % 100
        if remainder == 0:
            return f"100.wav+{hundreds}.wav"
        else:
            return f"100.wav+{hundreds}.wav+KƐ.wav+{pronounce_number(remainder)}"
    
    elif number < 1000000:
        thousands = number // 1000
        remainder = number % 1000
        if remainder == 0:
            return f"1000.wav+{pronounce_number(thousands)}"
        else:
            return f"1000.wav+{pronounce_number(thousands)}+KƐ.wav+{pronounce_number(remainder)}"
    
    else:
        millions = number // 1000000
        remainder = number % 1000000
        if remainder == 0:
            return f"1000000.wav+{pronounce_number(millions)}"
        else:
            return f"1000000.wav+{pronounce_number(millions)}+KƐ.wav+{pronounce_number(remainder)}"

# Function to simulate pronunciation by printing .wav files
def simulate_pronunciation(number):
    pronunciation = pronounce_number(number)
    print(f"Number {number} is pronounced as: {pronunciation}")

# Test cases
simulate_pronunciation(5)        # Simple case
simulate_pronunciation(21)       # 20-90.wav + 2.wav + KƐ.wav + 1.wav
simulate_pronunciation(100)      # 100.wav
simulate_pronunciation(123)      # 100.wav + 1.wav + 20-90.wav + 2.wav + KƐ.wav + 3.wav
simulate_pronunciation(1000)     # 1000.wav
simulate_pronunciation(2000)     # 1000.wav + 2.wav
simulate_pronunciation(2015)     # 1000.wav + 2.wav + 10.wav + KƐ.wav + 5.wav
simulate_pronunciation(3468567)  # 1000000.wav + 3.wav + 1000.wav + 100.wav + 4.wav + 20-90.wav + 6.wav + KƐ.wav + 8.wav + 100.wav + 5.wav + 20-90.wav + 6.wav + KƐ.wav + 7.wav