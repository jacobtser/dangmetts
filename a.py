...
# Mapping of characters to corresponding audio files
   valid_word_file_map = {
...
def generate_wav_sequence(number):
    ...
    # Function to handle numbers between 1000000 and above
...
# Function to split text into phonemes
...
def concatenate_audio(files, speed=1.0):
    ...
    try:
        ...
        speed = 1.0  # Default speed if invalid
    ...
    for file in files:
        ...
        if os.path.exists(filepath):
            ...
            try:
                data, fs = sf.read(filepath)
                print(f"Sample rate (fs): {fs} (type: {type(fs)})")  # Debugging
                print(f"Speed: {speed}")  # Debugging
                print(f"Data shape: {data.shape}")  # Debugging
                print(f"Data type: {data.dtype}")  # Debugging

                if sample_rate is None:
                    sample_rate = int(fs * speed)
                    print(f"Calculated sample rate: {sample_rate}")  # Debugging

                # Ensure the data is in the correct format (e.g., float32)
                if data.dtype != np.float32:
                    data = data.astype(np.float32)

                # Add a small amount of silence between files (e.g., 0.1 seconds)
                silence = np.zeros(int(0.1 * sample_rate))  # 0.1 seconds of silence
                ...
                print(f"Error reading file {filepath}: {e}")  # Debugging
        else:
...
def tts():
    ...
    try:
        print(f"Input text: {text}")  # Debugging
        tokens = re.findall(r'\d+|\D+', text)
        print(f"Tokens: {tokens}")  # Debugging
        audio_files = []
        for token in tokens:
            if token.isdigit():
                print(f"Processing number: {token}")  # Debugging
                sequence = generate_wav_sequence(int(token))
                print(f"Number {token} maps to audio files: {sequence}")  # Debugging
                if not sequence:
                    print(f"Warning: No audio files found for number: {token}")
                audio_files.extend(sequence)
            else:
                print(f"Processing text: {token}")  # Debugging
                phonemes = split_into_phonemes(token, valid_word_file_map)
                print(f"Text {token} maps to audio files: {phonemes}")  # Debugging
                if not phonemes:
                    print(f"Warning: No audio files found for text: {token}")
                audio_files.extend(phonemes)

        print(f"Final audio files to concatenate: {audio_files}")  # Debugging
        if not audio_files:
