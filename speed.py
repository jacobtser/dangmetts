...
# Function to play audio files at 1.5x speed
    ...
    for file in files:
        ...
        try:
            if os.path.exists(filepath):
                ...
                data, fs = sf.read(filepath)  # Load the audio file
                # Increase playback speed by 1.5x
                new_fs = int(fs * 2.0)  # New sample rate
                sd.play(data, new_fs)  # Play audio at the new sample rate
...
def generate_wav_sequence(number):
    ...
    def handle_hundreds(num):
        ...
        if remainder == 0:
            ...
            return [get_wav(100), get_wav(hundreds), ke_wav] + generate_wav_sequence(remainder)
    ...
    def handle_thousands(num):
        ...
        if remainder == 0:
            ...
            return [get_wav(1000)] + generate_wav_sequence(thousands) + [ke_wav] + generate_wav_sequence(remainder)
    ...
    def handle_millions(num):
        ...
        if remainder == 0:
            ...
            return [get_wav(1000000)] + generate_wav_sequence(millions) + [ke_wav] + generate_wav_sequence(remainder)
    ...
    while i < len(text):
        ...
        for j in range(min(len(text), i + 5), i, -1):
            ...
            if substring in phoneme_map:
                ...
                break
        if not matched:
            logging.warning(f"Wa tui: {text[i]} he blɔ nyã aloo pí Dãngme pɛlɔ(i)")
            i += 1
    return phonemes

# Function to play phonemes
def play_phonemes(phonemes, phoneme_map):
    for phoneme in phonemes:
        audio_file = phoneme_map.get(phoneme)
        if audio_file:
            play_audio([audio_file])
        else:
            logging.warning(f"Wa tui : {phoneme} he blɔ nyã lolo aloo pí Dãngme")

# Function to process mixed input (words and numbers)
def process_mixed_input(text):
    ...
    for token in tokens:
        if token.isdigit():  # If the token is a number
            ...
            play_phonemes(phonemes, valid_word_file_map)
...
while True:
    ...
    if user_input == 'q':
        ...
        break
    process_mixed_input(user_input)